...
onFileSelected(event: any) {
  ...
  loadSuccessors(equipement: Equip): void {
    ...
    // Use the new method with inter-department transitions for Juridique
  ...
  passerEtatSuivant(equipement: Equip): void {
    ...
    if (successors && successors.length > 0) {
      ...
      if (nextState.responsable !== 'JURIDIQUE') {
        this.showNotification('error', `Vous ne pouvez pas passer à l'état "${nextState.titre}" car il est destiné au département ${nextState.responsable}`);
        return;
      }
