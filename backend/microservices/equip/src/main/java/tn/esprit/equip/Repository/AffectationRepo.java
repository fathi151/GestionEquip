...
public interface AffectationRepo  extends JpaRepository<Affectation,Long> {
    ...
    @Query("SELECT a.equipement FROM Affectation a " +
            "LEFT JOIN a.equipement.pannes p " +
            "LEFT JOIN p.etatActuel e " +
            "WHERE a.verrou = 'affecter' " +
            "AND (p IS NULL OR e.responsable = 'DSI' OR e.responsable = 'DAG' OR e.responsable = 'JURIDIQUE') ")
    Page<Equipement> findEquipementsDSI(Pageable pageable);



    @Query("SELECT a.equipement FROM Affectation a " +
            "JOIN a.equipement.pannes p " +
            "JOIN p.etatActuel e " +
            "WHERE a.verrou = 'affecter' " +
            "AND (e.responsable = 'DAG' OR e.responsable = 'JURIDIQUE') " +
            "AND p.end = 0")
