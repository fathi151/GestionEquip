...
public interface AffectationRepo  extends JpaRepository<Affectation,Long> {
    ...
    @Query("SELECT a.equipement FROM Affectation a LEFT JOIN a.equipement.panne p LEFT JOIN p.etatActuel e WHERE a.verrou = 'affecter' AND (p IS NULL OR e.responsable = 'DSI' OR e.responsable = 'DAG' OR e.responsable = 'JURIDIQUE')")
