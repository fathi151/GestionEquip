...
@Component({
  selector: 'app-juridique-equi',
  templateUrl: './juridique-equi.component.html',
  styleUrls: ['./juridique-equi.component.css']
})
export class JuridiqueEquiComponent {
  ...
  loadEquipements(page: number): void {
    ...
    this.authservice.getJuridiqueEquipements(page, this.pageSize).subscribe({
      next: (res) => {
        this.equiements = res.content;
        this.totalPages = res.totalPages;
        this.fetchUtilisateurs(this.equiements);
...
loadSuccessors(equipement: Equip): void {
  if (!equipement.pannes || !equipement.pannes.length) {
    this.successorStates[equipement.idEqui] = [];
    return;
  }

  const lastPanne = equipement.pannes[equipement.pannes.length - 1];
  if (!lastPanne) {
    this.successorStates[equipement.idEqui] = [];
    return;
  }
  ...
  if (lastPanne.etatActuel == null) {
    console.debug(`Equip ${equipement.idEqui} last panne has no etatActuel`);
    this.successorStates[equipement.idEqui] = [];
    return;
  }
  if (typeof lastPanne.etatActuel === 'number') {
    currentStateId = lastPanne.etatActuel as number;
  } else if ((lastPanne.etatActuel as any).id != null) {
    currentStateId = (lastPanne.etatActuel as any).id;
  }

  if (!currentStateId) {
    console.debug(`Could not determine state id for equip ${equipement.idEqui}`, lastPanne.etatActuel);
    this.successorStates[equipement.idEqui] = [];
    return;
  }

  // Use the method with inter-department transitions for Juridique
  this.authservice.getSuccessorsJuridiqueWithTransitions(currentStateId).subscribe({
    next: (successors) => {
      this.successorStates[equipement.idEqui] = successors || [];
      console.log('Successeurs Juridique avec transitions:', successors);
    },
    error: (error) => {
...
passerEtatSuivant(equipement: Equip): void {
  ...
  if (successors && successors.length > 0) {
    ...
    // Vérification des permissions : JURIDIQUE ne peut passer qu'aux états JURIDIQUE
    if (nextState.responsable !== 'JURIDIQUE') {
      this.showNotification(
        'error',
        `Vous ne pouvez pas passer à l'état "${nextState.titre}" car il est destiné au département ${nextState.responsable}`
      );
      return;
    }
...
confirmTransition() {
  if (!this.transitionEquip || !this.transitionNextState) {
    return;
  }

  const lastPanne = this.getCurrentPanne(this.transitionEquip);
  if (!lastPanne) return;
  const panneId = lastPanne.id;
  ...
  // Return the active panne (end===0) or last panne
  getCurrentPanne(equipement: any) {
    if (!equipement || !equipement.pannes || !equipement.pannes.length) return null;
    return equipement.pannes.find((p: any) => p.end === 0) || equipement.pannes[equipement.pannes.length - 1];
  }

  // Return true when the equipment has no active panne (process finished) or no pannes at all
  isProcessFinished(equipement: any): boolean {
    if (!equipement) return true;
    if (!equipement.pannes || equipement.pannes.length === 0) return true;
    // If there is at least one panne with end === 0 it's still in process
    const hasActive = equipement.pannes.some((p: any) => p.end === 0);
    return !hasActive;
  }
