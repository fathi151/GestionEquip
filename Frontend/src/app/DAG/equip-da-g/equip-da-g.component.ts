...
// Get the next state for the last panne
...
getNextState(equipement: Equip): string {
  ...
  if (successors && successors.length > 0) {
    return successors[0].titre; // Return the first successor
  }

  return 'Aucun état suivant';
}

// Load successors for the last panne
loadSuccessors(equipement: Equip): void {
  ...
  // Pass to the next state (open modal to collect description)
  ...
  const lastPanne = equipement.pannes[equipement.pannes.length - 1];
  if (!lastPanne.etatActuel) return;
...
// Confirm transition
confirmTransition() {
  if (!this.transitionEquip || !this.transitionNextState) return;

  const currentPanne = this.getCurrentPanne(this.transitionEquip);
